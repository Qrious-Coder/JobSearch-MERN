...
- import Landing in App.tsx and render
...
- create custom-api, bad-request, not-found, index.tsx files
- add proper imports
- setup index.tsx just like in the front-end
- import {BadRequestError} in authController
- gotcha "errors/index.tsx"
...
- create AddJob,AllJobs,Profile,Stats,SharedLayout, index.tsx
- setup basic returns
...
- export all with index.tsx (just like components)
- import all pages in App.tsx
...
import { UnAuthenticatedError } from '../errors/index.tsx';
...
import { StatusCodes } from 'http-status-codes';
import { BadRequestError, NotFoundError } from '../errors/index.tsx';
...
import { UnAuthorizedError } from '../errors/index.tsx';
...
client/components/SearchContainer.tsx
...
```js
import { BadRequestError } from '../errors/index.tsx';
...
## Method 1: manually input headers.Authorization

```js
const res = await axios.patch(`/api/v1/auth/${endpoint}`, currentUser, {
  headers: {
    Authorization: `Bearer ${token}`,
  },
});
...
```js
export const patchUpdateUser = ({ currentUser, token }) => {
  axios({
    method: 'patch',
...
## Method 2: create an instance of Axios
...
const res = await authApi
  .fetchAuth(token)
  .patch(`/auth/updateUser`, currentUser);
```
#### note
```javascript
